...
import { 
  ...
  FaClock, FaCogs, FaCube, FaMicrochip, FaTerminal, FaFileExcel, 
  FaLaptopCode, FaChartPie, FaTable, FaChevronLeft
} from 'react-icons/fa';
  ...
  return (
    <div className="glass-card hover-y transition-all h-100 training-card">
      <div className="d-flex justify-content-between align-items-start">
        <div className={`glass-icon mb-3 ${colorClass}`} style={{ width: '60px', height: '60px', fontSize: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <IconComponent />
        </div>
...
const TrainingHub = ({ onBack }) => {
  ...
  const filteredCourses = activeFilter === 'All'
    ? courses
    : courses.filter(course => course.division.includes(activeFilter));
  ...
  return (
    <div className="page-container position-relative container py-5" style={{ minHeight: '100vh' }}>
      ...
      <AnimatePresence mode='wait'>
        ...
        >
          {filteredCourses.map((course) => (
            <motion.div key={course.id} className="col-md-6 col-lg-4" variants={itemVariants}>
              <CourseCard {...course} />
            </motion.div>
          ))}
        </motion.div>
